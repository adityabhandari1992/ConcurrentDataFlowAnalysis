...
public class LiveMain {
	...
	protected static void performLiveVariablesAnalysis(SootMethod m) {	    
	    ...
	    if (m.getName().equals("run") && count == threadCount-1 ) {
			...
			DotGraph combinedDg = combinedCfgtdg.drawCFG(combinedCFG, combinedCFG.getBody());
			...
			*/
			...
			a = new LiveVariables(combinedCFG);
	...
	private static void printCallGraph(CallGraph cg, SootMethod m) {
	    ...
	    while (targets.hasNext()) {
	        ...
	        if (trgt.getName().equals("run") && count < threadCount) performLiveVariablesAnalysis(trgt);
	    }
	}
	...
	public static void main(String[] args) {
	    ...
	    add(new Transform("wjtp.liveVariables", new SceneTransformer() {
			...
			protected void internalTransform (String phaseName, Map options) {
			    ...
			    printCallGraph(cg, m);
			    performLiveVariablesAnalysis(m);
			    System.out.println("Exiting CFG transformer");
			    
		  	}
		}));
	    
	    Options.v().set_main_class("LiveVariablesInput");
